...
#define OS_FSYS 480000000L  // MCU core frequency del STM32F767
...
void HIF_UART_Init(uint32_t Baudrate, UART_ON_TX_FUNC_P cbOnTx, UART_ON_RX_FUNC_P cbOnRx) {
  ...
  RCC->APB1HENR= (1 << 18);       // Habilitar reloj USART3 (APB1)

  RCC->AHB4ENR |= (1 << 3);        // Habilitar reloj GPIO D


  // ПОДКЛЮЧАТЬ ПИНЫ В CubeMX!!!
